# nf-core/demultiplex: Configuration for other clusters
...
Nextflow has [excellent integration](https://www.nextflow.io/docs/latest/docker.html) with Docker, and beyond installing the two tools, not much else is required - nextflow will automatically fetch the [nfcore/demultiplex](https://hub.docker.com/r/nfcore/demultiplex/) image that we have created and is hosted at dockerhub at run time.

To add docker support to your own config file, add the following:

```nextflow
docker.enabled = true
process.container = "nfcore/demultiplex"
```
...
process.container = "shub://nf-core/demultiplex"
```
...
```nextflow
singularity.enabled = true
process.container = "/path/to/nf-core-demultiplex.simg"
```
