...
By default, when trimming is enabled (`--trim_fastq true`), both the raw (untrimmed) and trimmed FASTQ files are published to the output directory. This can double storage usage for FASTQ files. To publish only the trimmed FASTQs and exclude the raw files, set `--publish_raw_fastq false`. The trimmed FASTQs are identifiable by the `.fastp.` infix in their filename (e.g., `Sample1_R1.fastp.fastq.gz`).
