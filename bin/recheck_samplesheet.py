...
for index, row in SS_new_problem_ids.iterrows():
    ...
    if update_idx_val['Sample_ID'] == row['Sample_ID'] and update_idx_val['index'] == row['index'] and update_idx_val['Lane'] == row['Lane']:
            if update_idx_val['index2'] == row['index2'] or (pd.isna(update_idx_val['index2']) and pd.isna(row['index2'])):
                test_result = 'fail'
